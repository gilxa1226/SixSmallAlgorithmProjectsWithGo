...
func NewLinearProbingHashTable(capacity int) *QuadraticProbingHashTable {
	...
	return &QuadraticProbingHashTable{capacity: capacity, employees: empArr}
}
...
func (hashTable *QuadraticProbingHashTable) find(name string) (int, int) {
	...
	for i := 0; i < hashTable.capacity; i++ {
		idx := (hash + i*i) % hashTable.capacity
		if hashTable.employees[idx] == nil {
			if deleted_index >= 0 {
...
func (hashTable *QuadraticProbingHashTable) dump_concise() {
...
func (hashTable *QuadraticProbingHashTable) ave_probe_sequence_length() float32 {
	total_length := 0
	num_values := 0

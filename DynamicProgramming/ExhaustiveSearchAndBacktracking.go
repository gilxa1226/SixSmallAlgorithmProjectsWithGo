...
func do_exhaustive_search(items []Item, allowed_weight, next_index int) ([]Item, int, int) {
	var add_weight, add_calls, rem_weight, rem_calls int
	var add_items, rem_items []Item

	if next_index >= len(items) {
		final_state := copy_items(items)
		final_value := solution_value(items, allowed_weight)
		return final_state, final_value, 1
	}

	items[next_index].is_selected = true
	add_items, add_weight, add_calls = do_exhaustive_search(items, allowed_weight, next_index+1)

	items[next_index].is_selected = false
	rem_items, rem_weight, rem_calls = do_exhaustive_search(items, allowed_weight, next_index+1)

	if solution_value(add_items, allowed_weight) > solution_value(rem_items, allowed_weight) {
		return add_items, add_weight, add_calls + 1
	} else {
		return rem_items, rem_weight, rem_calls + 1
	}
}

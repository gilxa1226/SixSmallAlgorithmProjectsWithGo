...
const num_items = 200 // A reasonable value for exhaustive search.
...
func dynamic_programming(items []Item, allowed_weight int) ([]Item, int, int) {
	num_items := len(items)
	solution_value := make([][]int, num_items)
	prev_weight := make([][]int, num_items)

	for idx := 0; idx < num_items; idx++ {
		solution_value[idx] = make([]int, allowed_weight+1)
		prev_weight[idx] = make([]int, allowed_weight+1)
	}

	// Initialize row 0.
	for w := 0; w <= allowed_weight; w++ {
		if items[0].weight <= w {
			// item[0] fits.
	...
	for i := 1; i < num_items; i++ {
		for w := 0; w <= allowed_weight; w++ {
			// calculate the value without i
			value_without := solution_value[i-1][w]
			// calculate the value with i
			value_with := 0
			if items[i].weight <= w {
				value_with = solution_value[i-1][w-items[i].weight] + items[i].value
			}

			// see which value is better.
			if value_without >= value_with {
				// Don't add item
				solution_value[i][w] = value_without
				prev_weight[i][w] = w
			} else {
				// Add item
				solution_value[i][w] = value_with
				prev_weight[i][w] = w - items[i].weight
			}
	...
	return items, solution_value[num_items-1][allowed_weight], 1
}
...
func main() {
	...
	if num_items > 150 { // Only use Rod's technique if num_items <= 85.
